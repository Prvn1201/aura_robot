...
class TrafficLightDetector(Node):
    ...
    def _get_hsv_ranges(self) -> Dict[str, List[Tuple[np.ndarray, np.ndarray]]]:
    ...
    def color_callback(self, msg: Image) -> None:
        ...
        try:
            ...
            min_r = self._param_cache['min_circle_radius']
            max_r = self._param_cache['max_circle_radius']
            ...
            if self._param_cache['show_debug']:
                self._draw_debug(roi, hsv, best_color, ranges, required)
    ...
    def _draw_debug(self, roi, hsv, best_color, ranges, required) -> None:
        ...
        if best_color in COLOR_MAP:
            ...
            b_mask = cv2.morphologyEx(b_mask, cv2.MORPH_OPEN, self._kernel)
            b_mask = cv2.morphologyEx(b_mask, cv2.MORPH_CLOSE, self._kernel)
